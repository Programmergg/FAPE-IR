...
from fapeir.models.qwen2vl.configuration_fapeir_qwen2vl import FAPEIRQwen2VLConfig
from fapeir.models.modeling_fapeir_denoise_tower_moe import FAPEIRDenoiseTower

class FAPEIRQwen2VLModel(Qwen2VLModel):
    def __init__(self, config: FAPEIRQwen2VLConfig):
        super().__init__(config)
        self.config = config

class FAPEIRQwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]
    config_class = FAPEIRQwen2VLConfig

    def __init__(self, config: FAPEIRQwen2VLConfig):
        super().__init__(config)
        self.visual = Qwen2VisionTransformerPretrainedModel._from_config(config.vision_config)
        print("visual init done")
        self.model = FAPEIRQwen2VLModel(config)
