...
from fapeir.models.configuration_fapeir import FAPEIRConfig
from transformers.modeling_outputs import CausalLMOutputWithPast
from fapeir.models.modeling_fapeir_vision_tower import FAPEIRVisionTower
from fapeir.models.modeling_fapeir_denoise_tower_moe import FAPEIRDenoiseTower


class FAPEIRQwen2Model(Qwen2Model):
    def __init__(self, config: FAPEIRConfig):
        super().__init__(config)
        self.config = config


class FAPEIRQwen2ForCausalLM(Qwen2PreTrainedModel, GenerationMixin):
    config_class = FAPEIRConfig

    def __init__(self, config: FAPEIRConfig):
        super().__init__(config)
        self.model = FAPEIRQwen2Model(config)
        self.vision_tower = FAPEIRVisionTower(config.vision_tower)
