...
def validate_single_dataset(
        ...
        for batch_idx, batch in enumerate(test_dataloader):
            ...
            for i in range(B):
                ...
                if is_realsr_dataset:
                    sample_result["camera_type"]  = batch.get("camera_type",  ["unknown"])[i] if "camera_type" in batch else "unknown"
                    sample_result["scale_factor"] = batch.get("scale_factor", [1])[i]         if "scale_factor" in batch else 1
                    sample_result["lr_path"]      = batch.get("lr_path",      ["unknown"])[i] if "lr_path" in batch else "unknown"
                    sample_result["hr_path"]      = batch.get("hr_path",      ["unknown"])[i] if "hr_path" in batch else "unknown"
                elif is_weather_dataset:
                    sample_result["weather_type"] = batch.get("weather_type", ["unknown"])[i] if "weather_type" in batch else "unknown"
                    sample_result["input_path"]   = batch.get("input_path",   ["unknown"])[i] if "input_path" in batch else "unknown"
                    sample_result["gt_path"]      = batch.get("gt_path",      ["unknown"])[i] if "gt_path" in batch else "unknown"
                elif is_generic_dataset:
                    sample_result["dataset_name"] = batch.get("dataset_name", ["unknown"])[i] if "dataset_name" in batch else "unknown"
                    sample_result["input_path"]   = batch.get("input_path",   ["unknown"])[i] if "input_path" in batch else "unknown"
                    sample_result["gt_path"]      = batch.get("gt_path",      ["unknown"])[i] if "gt_path" in batch else "unknown"
    ...
    if timing_records:
        ...
        accelerator.print(f"  Average postprocess time: {avg_postprocess_time:.3f}s ({# Attach dataset-specific metadata/avg_total_time*100:.1f}%)")
